...
// AI 代理请求（Spec §6.8：只读取当前用户配置，不接受请求体携带 Key/Endpoint）
router.post('/proxy', authMiddleware, async (req, res) => {
  ...
  if (!apiKey || !endpoint) {
    return res.status(400).json({ error: '请先在「AI设置」中配置 API Endpoint 和 Key', requireConfig: true });
  }
...
// 嵌入生成API（Spec §6.8：必须登录且只读取当前用户配置，禁止请求体携带 Key）
router.post('/embedding', authMiddleware, async (req, res) => {
  ...
  const resolvedKey = userConfig?.api_key;
  const resolvedEndpoint = userConfig?.embedding_endpoint;
  const resolvedModel = userConfig?.embedding_model || 'text-embedding-v3';

  if (!resolvedKey || !resolvedEndpoint) {
    return res.status(400).json({ error: '请先在「AI设置」中配置嵌入模型 Endpoint 和 Key', requireConfig: true });
  }
