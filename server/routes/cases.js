...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    // 排除 embedding 列：pgvector 向量序列化为 JSON 数组会膨胀 ~8 倍（4KB→32KB）
    ...
    const casesWithDetails = casesResult.rows.map(c => ({
      ...
      entities: entitiesByCase.get(c.id) || [],
      relations: relationsByCase.get(c.id) || []
    }));
...
router.get('/export-all', authMiddleware, async (req, res) => {
  try {
    const { format, schemaId } = req.query;

    // 优化：3 个并行查询代替 N+1（原来每个 case 2 次查询）
    // 支持 schemaId 过滤，只导出当前 Schema 下的案例
    const schemaFilter = schemaId ? 'WHERE schema_id = $1' : '';
    const schemaFilterParam = schemaId ? [parseInt(schemaId)] : [];
    const joinClause = schemaId ? 'JOIN cases ON case_entities.case_id = cases.id AND cases.schema_id = $1' : '';

    const [casesResult, entitiesResult, relationsResult] = await Promise.all([
      pool.query(`SELECT id, name, schema_id FROM cases ${schemaFilter} ORDER BY id`, schemaFilterParam),
      pool.query(`SELECT ce.case_id, ce.name, ce.entity_type, ce.properties, ce.color
                   FROM case_entities ce ${joinClause}
                   ORDER BY ce.case_id`, schemaFilterParam),
      pool.query(`SELECT cr.case_id, cr.relation_type,
                  ...
                  ${schemaId ? 'JOIN cases ON cr.case_id = cases.id AND cases.schema_id = $1' : ''}
                  ORDER BY cr.case_id`, schemaFilterParam),
    ]);
...
// 获取案例详情（优化：3 个查询并行执行；列瘦身理由同列表接口）
...
router.get('/:id/export', authMiddleware, async (req, res) => {
  try {
    ...
    const caseResult = await pool.query('SELECT id, name, schema_id, location, year, description, tags, created_at FROM cases WHERE id = $1', [id]);
    if (caseResult.rows.length === 0) {
      return res.status(404).json({ error: '案例不存在' });
    }
