...
function makeStatusHandler(targetType, allowedActions) {
  return async (req, res) => {
    const { id } = req.params;
    const { action } = req.params; // 路由参数里的动作（approve/reject/edit/restore）
...
// 实体审核
// ------------------------------------------------------------
router.post('/entity/:id/:action(approve|reject|edit|restore)', makeStatusHandler('entity', ['approve', 'reject', 'edit', 'restore']));
...
// 关系审核
// ------------------------------------------------------------
router.post('/relation/:id/:action(approve|reject|edit|restore)', makeStatusHandler('relation', ['approve', 'reject', 'edit', 'restore']));
