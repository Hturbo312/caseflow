      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <h4 className="text-xs font-semibold text-emerald-700">创建关系</h4>
          </div>
          <button
            type="button"
            onClick={onCloseMode}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-emerald-600">起点：</span>
            {relationSource ? (
              <span className="px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-[10px] font-medium">
                {relationSource.name}
              </span>
            ) : (
              <span className="text-[10px] text-gray-400 italic">点击选择</span>
            )}
          </div>
          <ArrowRight className="w-3 h-3 text-gray-400" />
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-emerald-600">终点：</span>
            {relationTarget ? (
              <span className="px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-[10px] font-medium">
                {relationTarget.name}
              </span>
      ...
      <AnimatePresence>
        {relationTypeSelectorOpen && relationSource && relationTarget && (
          ...
          >
            ...
            >
              <h3 className="text-lg font-semibold mb-4">选择关系类型</h3>
              <div className="space-y-2 max-h-60 overflow-y-auto">
                ...
                {(!currentSchema?.relations || currentSchema.relations.length === 0) && (
                  <p className="text-sm text-gray-500 text-center py-4">
                    当前 Schema 没有定义关系类型，请先在 Schema 管理中添加
                  </p>
                )}
              </div>
